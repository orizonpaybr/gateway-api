...
class FyhubMtlsOptions
{
    ...
    public static function build(): array
    {
        ...
        if ($format === 'pfx') {
            ...
            self::assertReadable($pfx, 'FYHUB_CERT_PFX_PATH');
        ...
        self::assertReadable($pem, 'FYHUB_CERT_PEM_PATH');
        self::assertReadable($key, 'FYHUB_CERT_KEY_PATH');
    ...
    private static function assertReadable(string $path, string $envKey): void
    {
        if (! is_readable($path)) {
            throw new \RuntimeException(
                "Certificado mTLS FYHUB QR ({$envKey}) não encontrado ou sem permissão de leitura: {$path}"
            );
        }
    }

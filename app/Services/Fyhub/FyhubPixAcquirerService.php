...
class FyhubPixAcquirerService implements PixAcquirerInterface
{
    ...
    public function createCharge(
        ...
        try {
            ...
            // Copia-e-cola EMV. O /cob às vezes retorna sob nomes diferentes; a doc
            // do cob imediato omite o campo. Preserva a prioridade histórica
            // (pixCopiaECola / dadosQR) e cai em nomes alternativos validando EMV.
            $pixCopiaECola = $body['pixCopiaECola']
                ?? $body['dadosQR']['pixCopiaECola']
                ?? null;

            if (! (is_string($pixCopiaECola) && trim($pixCopiaECola) !== '')) {
                foreach (['copiaECola', 'emv', 'brcode', 'brCode', 'qrcode', 'textoImagemQRcode'] as $altKey) {
                    $alt = $body[$altKey] ?? null;
                    if (is_string($alt) && self::looksLikeEmv($alt)) {
                        $pixCopiaECola = $alt;
                        break;
                    }
                }
            ...
            if (is_string($pixCopiaECola) && trim($pixCopiaECola) !== '') {
                ...
                // Sem EMV na resposta: location é só a URL do payload, NÃO um
                // copia-e-cola pagável. Loga pra pegar no teste ao vivo.
                $brCode = $body['location'] ?? ($body['loc']['location'] ?? null);
                if (! (is_string($brCode) && self::looksLikeEmv($brCode))) {
                    Log::warning('[FYHUB][CHARGE] Resposta sem copia-e-cola EMV; brCode caiu na location (validar campo no teste)', [
                        'txid' => $resolvedTxid,
    ...
    /**
     * Heurística: string parece um copia-e-cola Pix EMV (BR Code) e não uma URL de location.
     */
    private static function looksLikeEmv(string $value): bool
    {
        $t = trim($value);

        return str_starts_with($t, '000201') || stripos($t, 'br.gov.bcb.pix') !== false;
    }

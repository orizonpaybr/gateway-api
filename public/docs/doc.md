...
O HKPAY Gateway é uma solução completa para integração de pagamentos, oferecendo múltiplos adquirentes, sistema de taxas flexível, controle de IPs e webhooks em tempo real.
...
```http
POST https://demo.hkpay.shop/api/wallet/deposit/payment
...
```http
POST https://demo.hkpay.shop/api/wallet/saque/payment
```
...
```bash
curl -X GET "https://demo.hkpay.shop/api/card/payment/22368873?token=seu_token&secret=sua_secret"
```
...
## Gateway Unificado HKPAY

O HKPAY é um gateway unificado que simplifica toda a complexidade dos pagamentos:
...
O HKPAY gerencia automaticamente toda a complexidade por trás dos pagamentos. Você integra uma vez e recebe todos os benefícios de múltiplos processadores sem se preocupar com configurações técnicas.
...
- **Email**: suporte@HKPAY.com
- **Documentação**: https://demo.hkpay.shop/documentacao
- **Demo**: https://demo.hkpay.shop

---

**HKPAY Gateway** - Soluções completas em pagamentos

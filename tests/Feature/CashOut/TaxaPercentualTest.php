...
class TaxaPercentualTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Piso determinístico: Treeal R$ 0,03
    ...
    {
        $user = $this->makeUser(['taxa_percentual_deposito' => 2.00]);

        // 2% de R$ 1,00 = R$ 0,02, abaixo do piso de R$ 0,03 → cobra o piso
        $r = TaxaFlexivelHelper::calcularTaxaDeposito(1.00, $this->setting, $user, 'treeal');

        $this->assertEqualsWithDelta(0.03, $r['taxa_cash_in'], 0.0001);
    }
    ...
    {
        $user = $this->makeUser(['taxa_percentual_pix' => 2.00]);

        // 2% de R$ 1,00 = R$ 0,02, abaixo do piso de R$ 0,03 → cobra o piso
        $r = TaxaSaqueHelper::calcularTaxaSaque(1.00, $this->setting, $user, true, false, 'treeal');

        $this->assertEqualsWithDelta(0.03, $r['taxa_cash_out'], 0.0001);
    }

...
/**
 * Testes de Segurança - Extrato (Depósitos e Saques)
 * 
 * Cobre:
 * - SQL Injection
 ...
 * - Date Manipulation
 */
class ExtratoSecurityTest extends TestCase
{
    use RefreshDatabase;
    ...
    /**
     * Helper para criar SolicitacoesCashOut
     */
    ...
    public function test_should_prevent_sql_injection_in_search_param(): void
    {
        ...
        foreach ($sqlInjectionPayloads as $payload) {
            ...
            ])->getJson("/api/extrato?busca=" . urlencode($payload));
    ...
    public function test_should_prevent_sql_injection_in_date_params(): void
    {
        ...
        foreach ($sqlInjectionPayloads as $payload) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?data_inicio=" . urlencode($payload));
    ...
    public function test_should_prevent_sql_injection_in_type_param(): void
    {
        ...
        foreach ($sqlInjectionPayloads as $payload) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?tipo=" . urlencode($payload));
    ...
    public function test_should_prevent_xss_in_search_param(): void
    {
        ...
        foreach ($xssPayloads as $payload) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?busca={$encodedPayload}");
    ...
    /**
     * Teste: Deve exigir autenticação para acessar extrato
     */
    public function test_should_require_authentication_for_extrato(): void
    {
        $response = $this->getJson('/api/extrato');
    ...
    /**
     * Teste: Deve prevenir acesso a extrato de outros usuários (IDOR)
     */
    public function test_should_prevent_idor_access_to_other_users_extrato(): void
    {
        // Criar transações para o outro usuário
        ...
        // Tentar acessar extrato usando token do primeiro usuário
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson('/api/extrato');
    ...
    public function test_should_validate_period_parameters(): void
    {
        ...
        foreach ($invalidPeriods as $period) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?periodo={$encodedPeriod}");
    ...
    public function test_should_validate_pagination_limits(): void
    {
        ...
        foreach ($invalidLimits as $limit) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?limit={$limit}");
    ...
    public function test_should_validate_date_format(): void
    {
        ...
        foreach ($invalidDates as $date) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?data_inicio={$encodedDate}");
    ...
    public function test_should_validate_type_parameters(): void
    {
        ...
        foreach ($invalidTypes as $type) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/extrato?tipo={$encodedType}");
    ...
    public function test_should_not_expose_other_users_data(): void
    {
        ...
        // Buscar extrato do usuário autenticado
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson('/api/extrato');
    ...
    public function test_should_validate_date_manipulation(): void
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson("/api/extrato?data_inicio={$futureDate}&data_fim={$futureDate}");
    ...
    public function test_should_validate_invalid_date_range(): void
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson("/api/extrato?data_inicio={$startDate}&data_fim={$endDate}");

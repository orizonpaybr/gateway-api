...
/**
 * Testes de Segurança - Transações Pendentes
 * 
 * Cobre:
 * - SQL Injection
 ...
 * - Date Manipulation
 */
class PendingTransactionsSecurityTest extends TestCase
{
    use RefreshDatabase;
    ...
    /**
     * Helper para criar Solicitacoes (depósitos pendentes)
     */
    ...
    /**
     * Helper para criar SolicitacoesCashOut (saques pendentes)
     */
    ...
    public function test_should_prevent_sql_injection_in_search_param(): void
    {
        ...
        foreach ($sqlInjectionPayloads as $payload) {
            ...
            ])->getJson("/api/transactions?status=PENDING&busca=" . urlencode($payload));
    ...
    public function test_should_prevent_sql_injection_in_date_params(): void
    {
        ...
        foreach ($sqlInjectionPayloads as $payload) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/transactions?status=PENDING&data_inicio=" . urlencode($payload));
    ...
    /**
     * Teste: Deve prevenir SQL Injection em parâmetros de tipo
     */
    public function test_should_prevent_sql_injection_in_type_param(): void
    {
        $sqlInjectionPayloads = [
            "' OR '1'='1",
            "deposito' OR '1'='1'--",
        ...
        foreach ($sqlInjectionPayloads as $payload) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/transactions?status=PENDING&tipo=" . urlencode($payload));
    ...
    public function test_should_prevent_xss_in_search_param(): void
    {
        ...
        foreach ($xssPayloads as $payload) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/transactions?status=PENDING&busca={$encodedPayload}");
    ...
    /**
     * Teste: Deve exigir autenticação para acessar transações pendentes
     */
    public function test_should_require_authentication_for_pending_transactions(): void
    {
    ...
    /**
     * Teste: Deve prevenir acesso a transações pendentes de outros usuários (IDOR)
     */
    public function test_should_prevent_idor_access_to_other_users_pending_transactions(): void
    {
        // Criar transação pendente para o outro usuário
        ...
        // Tentar acessar transações pendentes usando token do primeiro usuário
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ...
        // Não deve conter transações do outro usuário
        if (is_array($data)) {
            foreach ($data as $transaction) {
                // Verificar que não há transações do outro usuário
                $this->assertNotEquals('TXN_OTHER_USER', $transaction['transaction_id'] ?? '');
            }
        }
    }
    ...
    public function test_should_validate_pagination_limits(): void
    {
        $invalidLimits = [
            -1,
            0,
            10000, // Muito grande (máximo é 50)
            "' OR '1'='1",
        ];
        ...
        foreach ($invalidLimits as $limit) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/transactions?status=PENDING&limit={$limit}");
    ...
    public function test_should_validate_date_format(): void
    {
        ...
        foreach ($invalidDates as $date) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/transactions?status=PENDING&data_inicio={$encodedDate}");
    ...
    public function test_should_validate_type_parameters(): void
    {
        ...
        foreach ($invalidTypes as $type) {
            ...
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson("/api/transactions?status=PENDING&tipo={$encodedType}");
    ...
    {
        // Criar transação pendente de teste
        $this->createSolicitacao([
    ...
    {
        // Criar transação pendente para o outro usuário com valor alto
        $this->createSolicitacao([
            'user_id' => $this->otherUser->username,
        ...
        // Buscar transações pendentes do usuário autenticado
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson('/api/transactions?status=PENDING');
    ...
    public function test_should_validate_date_manipulation(): void
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson("/api/transactions?status=PENDING&data_inicio={$futureDate}&data_fim={$futureDate}");
    ...
    public function test_should_validate_invalid_date_range(): void
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson("/api/transactions?status=PENDING&data_inicio={$startDate}&data_fim={$endDate}");
    ...
    public function test_should_implement_rate_limiting(): void
    {
        ...
        for ($i = 0; $i < 50; $i++) {
            $response = $this->withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
            ])->getJson('/api/transactions?status=PENDING&page=' . $i);

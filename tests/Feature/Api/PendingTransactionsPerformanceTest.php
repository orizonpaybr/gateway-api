...
/**
 * Testes de Performance e Concorrência - API de Transações Pendentes
 * 
 * Cobre:
 ...
 */
class PendingTransactionsPerformanceTest extends TestCase
{
    use RefreshDatabase;
    ...
    /**
     * Helper para criar múltiplas transações pendentes
     */
    private function createMultiplePendingTransactions(int $count, User $user): void
    {
        $transactions = [];
        $now = Carbon::now();
        ...
        for ($i = 0; $i < $count; $i++) {
            $transactions[] = [
                ...
                'descricao_transacao' => 'Transação pendente ' . $i,
    ...
    /**
     * Teste: Deve responder em menos de 500ms com 1000 transações pendentes
     */
    public function test_should_respond_under_500ms_with_1000_pending_transactions(): void
    {
        $this->createMultiplePendingTransactions(1000, $this->user);
    ...
    {
        $this->createMultiplePendingTransactions(100, $this->user);
    ...
    {
        $this->createMultiplePendingTransactions(500, $this->user);
    ...
    {
        $this->createMultiplePendingTransactions(10000, $this->user);
    ...
    public function test_should_optimize_queries_with_indexes(): void
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson('/api/transactions?status=PENDING&data_inicio=' . Carbon::now()->subDays(7)->format('Y-m-d'));
    ...
    {
        $this->createMultiplePendingTransactions(2000, $this->user);
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson('/api/transactions?status=PENDING&busca=TXN&data_inicio=' . Carbon::now()->subDays(30)->format('Y-m-d') . '&data_fim=' . Carbon::now()->format('Y-m-d'));
    ...
    {
        $this->createMultiplePendingTransactions(5000, $this->user);
        ...
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
        ])->getJson('/api/transactions?status=PENDING&page=250&limit=20');
    ...
    /**
     * Teste: Deve processar UNION eficientemente
     */
    public function test_should_process_union_efficiently(): void
    {
        // Criar depósitos e saques pendentes
        for ($i = 0; $i < 500; $i++) {
            Solicitacoes::create([
            ...
            SolicitacoesCashOut::create([
                ...
                'descricao_transacao' => 'Saque pendente ' . $i,
            ]);
        }
        ...
        $this->assertLessThan(1000, $duration, "UNION demorou {$duration}ms");
    }
}

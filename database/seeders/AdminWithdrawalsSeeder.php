...
class AdminWithdrawalsSeeder extends Seeder
{
    /**
     * Criar 30 saques para aprovação no dashboard admin
     * Diferentes status, tipos e valores para testar filtros e paginação
     */
    public function run(): void
    {
        ...
        $withdrawals = [];
        $statuses = ['PENDING', 'PENDING', 'PENDING', 'COMPLETED', 'CANCELLED']; // Mais pendentes
        $types = ['cpf', 'cnpj', 'email', 'phone', 'random'];
        
        for ($i = 1; $i <= 30; $i++) {
            $userId = $userIds[array_rand($userIds)];
            $amount = $this->randomAmount(500, 15000); // R$ 500 - R$ 15k
            $taxaCashOut = $amount * 0.025; // 2.5%
            $cashOutLiquido = $amount - $taxaCashOut;
            $status = $statuses[array_rand($statuses)];
            $daysAgo = rand(0, 30);
            $hoursAgo = rand(0, 23);
            $date = Carbon::now()->subDays($daysAgo)->subHours($hoursAgo);
            $type = $types[array_rand($types)];

            // Pendentes não têm ID de transação, aprovados sim
            $idTransaction = $status === 'COMPLETED' ? 'ADMIN-WD-' . str_pad($i, 4, '0', STR_PAD_LEFT) : null;
            $externalReference = 'ADMIN-WD-EXT-' . strtoupper(uniqid());

            // Executor: 50% manual, 50% automático
        ...
        // Limpar saques admin anteriores
        DB::table('solicitacoes_cash_out')
            ->where('externalreference', 'like', 'ADMIN-WD-EXT-%')
            ->delete();
        ...
        // Estatísticas
        $pendentes = count(array_filter($withdrawals, fn($w) => $w['status'] === 'PENDING'));
        $aprovados = count(array_filter($withdrawals, fn($w) => $w['status'] === 'COMPLETED'));
        $rejeitados = count(array_filter($withdrawals, fn($w) => $w['status'] === 'CANCELLED'));
        
        $this->command->info("30 saques criados para aprovação:");
        $this->command->info("  • Pendentes: {$pendentes}");
        $this->command->info("  • Aprovados: {$aprovados}");
        $this->command->info("  • Rejeitados: {$rejeitados}");
    }
